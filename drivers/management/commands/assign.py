...
#komponowanie macierzy a
        ...
        for i in range(len(ps)):
            for j in range(len(drs)):
                if assign[i][j] == 1:
                    ride = Ride(driver_id=drs[j].user_id, passenger_id=ps[i].user_id, date=drs[j].date, pick_up=ps[i].start, drop_off=ps[i].end)
                    ride.save()
                    print('passenger ' + str(i) + 'rides with driver ' + str(j))
